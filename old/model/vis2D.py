...
from loader import SuperResNpyDataset2,SuperResNpyDataset2D
...
###############################################################################################
def normalize(X,Y,M,S):
    X = (X - M[0])/(S[0]+1.0e-8)
    Y = (Y - M[1])/(S[1]+1.0e-8)
    return X,Y
#--
def denormalize(X,Y,M,S):
    X = X*(S[0]+1.0e-8)+M[0]
    Y = Y*(S[1]+1.0e-8)+M[1]
    return X,Y
###############################################################################################

# ## Vis 2D errors for training datas############################################################

# import os

# # Load the saved arrays
# errors_u = np.load("error_maps/error_maps_u.npy")  # shape (N, H, W)
# errors_v = np.load("error_maps/error_maps_v.npy")  # shape (N, H, W)

# # Stack to get shape (N, 2, H, W)
# combined_errors2 = np.stack([errors_u, errors_v], axis=1)  # Now shape: (N, 2, H, W)
# print("Combined error map shape:", combined_errors2.shape)

# # Save combined error map as a file
# os.makedirs("error_maps", exist_ok=True)
# np.save("error_maps/error_maps_combined2.npy", combined_errors2)

# # Create folder for images
# os.makedirs("error_maps/combined_visualizations2", exist_ok=True)

# # Visualize every 50th step
# for n in range(combined_errors2.shape[0]):
#     if n % 50 == 0:
#         u_map = combined_errors2[n, 0]  # (H, W)
#         v_map = combined_errors2[n, 1]  # (H, W)

#         plt.figure(figsize=(10, 4))

#         plt.subplot(1, 2, 1)
#         plt.imshow(u_map, cmap='viridis')
#         plt.title(f"U Error - Sample {n}")
#         plt.colorbar()

#         plt.subplot(1, 2, 2)
#         plt.imshow(v_map, cmap='viridis')
#         plt.title(f"V Error - Sample {n}")
#         plt.colorbar()

#         plt.tight_layout()
#         plt.savefig(f"error_maps/combined_visualizations2/sample_{n:03d}.png")
#         plt.close()
...
# random window
# mean = np.array([0.0350330168715593 , 0.03492018889668478 , 0.016571388 ])
# std = np.array([0.356975741 , 0.358710298 , 1.000951 ])



# Mean (LR) :: Std (LR) =  0.0350330168715593 0.3569757413317375
# Mean (HR) :: Std (HR) =  0.03492018889668478 0.35871029882324074
# Mean (eR) :: Std (eR) =  0.016571388 1.0009515
...
# # # random window
# mean = np.array([1.52539439413 , 4.86633303 , -1.1799857e-09 ])
# std = np.array([0.99999997198 , 0.999999972, 0.99999 ])
...
# model.load_state_dict(torch.load('../weights/MSE_train/MSE-order2/Reza_130.pth'))    # DSCMS HLH
...
## 2D  ##############################################################################################
# model1.load_state_dict(torch.load('../weights/MSE_train/MSE-order2/Reza_130.pth'))    # DSCMS HLH
# model1.load_state_dict(torch.load('../weights/train-test1/Reza_64.pth'))    # DSCMS HLH
# model1.load_state_dict(torch.load('../weights/2D_Model1/Reza_90.pth'))    #  2D_MOdel1 (even numbers)
model1.load_state_dict(torch.load('../weights/2D_check1/Reza_145.pth'))    #  2D_MOdel1 (even numbers)





# model1.load_state_dict(torch.load('../weights/train-DSCMS/Reza_110.pth'))  #DSCMS-HLH

# model2.load_state_dict(torch.load('../weights/2D/Reza_30.pth'))    # DSCMS HLH
# model2.load_state_dict(torch.load('../weights/2D_Model2/Reza_20.pth'))    #  2D_MOdel2 20
# model2.load_state_dict(torch.load('../weights/2D_Model2_MSE/Reza_50.pth'))    #  2D_MOdel2(MSE) 50
model2.load_state_dict(torch.load('../weights/2D_check2/Reza_268.pth'))    #  2D_check2 (even numbers)
...
dataset = SuperResNpyDataset2D(data_folder, lr_files, hr_files,er,0,mean,std)



test_loader = DataLoader(dataset, batch_size=1, shuffle=False)
print(np.shape(lr_files),np.shape(hr_files),np.shape(er),len(dataset))
...
with torch.no_grad():
    for i, (lr, hr,er) in enumerate(test_loader):
        print(i)

        # print(np.shape(lr))
        lr = lr.to(device)  # Move LR to device
        hr = hr.to(device)  # Move HR to device
        er = er.to(device)  # Move HR to device
        ...
        output1 = model1(lr_upsampled)
        # output1=(output1 - output1.min()) / (output1.max() - output1.min() )

        output2 = model2(lr_upsampled) 
        # output2 = torch.clamp(output2, 0.0, 1.0)

        # print("----",output1.mean(),output1.std(),output2.mean(),output2.std())

        # output2=(output2 - output2.min()) / (output2.max() - output2.min() )
        # print("----",output1.mean(),output1.std(),output2.mean(),output2.std())
        # exit()
        y1 = output1  # Model's output 
        y2 = output2   # error
        ...
        output1 = output1.cpu().squeeze().numpy()  # hr as a NumPy array (used for debugging)
        output2 = output2.cpu().squeeze().numpy()  # hr as a NumPy array (used for debugging)



        # print(np.shape(lr),np.shape(y1),np.shape(y2),np.shape(lr_resized),np.shape(output1),np.shape(output2))
        ...
        LR_SSIM_u.append(ssimLoss(hr,lr,0,i))
        LR_SSIM_v.append(ssimLoss(hr,lr,1,i))

        LR_PSNR_u.append(psnrLoss(hr,lr,0,i))
        LR_PSNR_v.append(psnrLoss(hr,lr,1,i))

        LR_lpips_u.append(lpipsLoss(hr,lr,0,i))
        LR_lpips_v.append(lpipsLoss(hr,lr,1,i))

        LR_FSIM_u.append(fsimLoss(hr,lr,0,i))
        LR_FSIM_v.append(fsimLoss(hr,lr,1,i))

        LR_EPI_u.append(epiLoss(hr,lr,0,i))
        LR_EPI_v.append(epiLoss(hr,lr,1,i))
        LR_TKE_u.append(LossTKE(hr,lr,0,i))
        LR_TKE_v.append(LossTKE(hr,lr,1,i))
        ...
        #low = low[:,:].reshape(4,int(low.shape[1]/4), 4, int(low.shape[2]/4), 4)
        ...
        # low[1,:,:] = returnVal(low[1,:,:],mean[0],std[0])
        # # print(mean[0],std[0],mean[1],std[1],mean[2],std[2])

        # # exit()

        # x8[0,:,:] = returnVal(x8[0,:,:],mean[2],std[2])
        # x8[1,:,:] = returnVal(x8[1,:,:],mean[2],std[2])
        # # # # print(mean[0],std[0],mean[1],std[1])

        # x8c[0,:,:] = returnVal(x8c[0,:,:],mean[1],std[1])
        # x8c[1,:,:] = returnVal(x8c[1,:,:],mean[1],std[1])

        # output[0,:,:] = returnVal(output[0,:,:],mean[1],std[1])
        # output[1,:,:] = returnVal(output[1,:,:],mean[1],std[1])


        # high[0,:,:] = returnVal(high[0,:,:],mean[1],std[1])
        # high[1,:,:] = returnVal(high[1,:,:],mean[1],std[1])
        # print(mean[0],std[0],mean[1],std[1])
        ...
        # low = low * (low_max - low_min) + low_min
        # x8c = x8c * (x8c_max - x8c_min) + x8c_min
        # x8 = x8 * (x8_max - x8_min) + x8_min
        ...
        if (i==day):
            ...
            plt.savefig("PLOT105.png",dpi=300,bbox_inches='tight')
...
os.makedirs("Histograms/His", exist_ok=True)
#==================================================================================================
loss_data_u = {
    "MSE": (LR_MSE_u, bilinear_MSE_u, bic_MSE_u, model_MSE_u),
    "SSIM": (LR_SSIM_u, bilinear_SSIM_u, bic_SSIM_u, model_SSIM_u),
...
for i, (data1_u, _, data3_u, data4_u) in loss_data_u.items():
    ...
    x = np.linspace(
        min(np.concatenate([data1_u, data3_u, data4_u])) - 1, 
        max(np.concatenate([data1_u, data3_u, data4_u])) + 1, 
        300
    )
    ...
    for (label, (mean, std)), color, data in zip(stats.items(), colors, [data1_u, data3_u, data4_u]):
...
for i, (data1_v, _, data3_v, data4_v) in loss_data_v.items():
    ...
    # Create a range for the x-axis
    x = np.linspace(
        min(np.concatenate([data1_v, data3_v, data4_v])) - 1, 
        max(np.concatenate([data1_v, data3_v, data4_v])) + 1, 
        300
    )
    ...
    for (label, (mean, std)), color, data in zip(stats.items(), colors, [data1_v,  data3_v, data4_v]):
        sns.histplot(data, label=f"{label} (μ={mean:.5f}, σ={std:.5f})", color=color, kde=True, bins=30, alpha=0.5)
