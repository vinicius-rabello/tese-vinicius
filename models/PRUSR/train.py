...
from models.PRUSR import config
...
class CombinedMSSSIMLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        return self.a * mse_loss + self.b * ssim_loss
...
def main():
    ...
    # Initialize model
    model = PRUSR(in_channels=2, out_channels=2)
    model = torch.nn.DataParallel(model)
    model = model.to(config.DEVICE)
    
    # Loss and optimizer
    criterion = CombinedMSSSIMLoss(a=10, b=1)
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        with open(config.ROOT_FOLDER + "output/logs/PRUSR_loss.txt", "a") as f:
