...
def main():
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        with open(config.ROOT_FOLDER + "output/logs/SRCNN_loss.txt", "a") as f:
        ...
        if val_loss < best_val_loss:
            ...
            save_checkpoint(model, optimizer, epoch+1, train_loss, val_loss,
                          config.ROOT_FOLDER + 'output/weights/SRCNN_best.pth')
        
        # Save checkpoint every 50 epochs
        if (epoch+1) % 50 == 0:
            save_checkpoint(model, optimizer, epoch+1, train_loss, val_loss,
                          config.ROOT_FOLDER + f'output/weights/SRCNN_epoch{epoch+1}.pth')
